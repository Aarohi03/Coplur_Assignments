1
select s.customer_id, sum(m.price) as amount_sent
from sales s
...
4
select customer_id,product_id, count(*) as total_times_ordered
from sales
where product_id = (
select product_id
from (
...
5
select customer_id, product_id,count(*) as total_order
from sales s1
where (
select count (*)
from sales s2
